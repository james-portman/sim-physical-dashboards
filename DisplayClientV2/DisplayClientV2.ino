...
// left plug:
// pin 14 - airbag - bottom right
// pin 17 - parking brake lamp - red LED on right of dash
// pin 18 - brake fluid warning (bottom)
// pin 19 - brake pad wear - (bottom) keeping it grounded turns it off, once on it stays on!

// middle plug:
// pin 6 - battery charge lamp (top)
// pin 12 - traction control centre/top/middle light
// pin 19 - oil warning (red) top of dash
// pin 26 - "check control" light (bottom)
...
void setSpeed(int mph) {
  if (mph < 1) {
    ...
    if (mph < 17) { mph = 17; }
    else if (mph > 155) { mph = 155; }
    int speedoutput = map(mph, 17, 155, 35, 315);
    e36SppedTone.play(speedoutput);
  }
}

void setFuel(int percent) {
  // acting up a lot, bad connection? needs power cycle to change really quickly?
  // also big delay on changes (expected)
  // 60  - 1/3 ish?
  // 80 - 1/2 or just under
  // 100 - 1/2 to 3/4
  // 150 - past full
  int fueloutput = map(percent, 0, 100, 10, 150);
  analogWrite(fuelPin, fueloutput); //Fuel Gauge
}

void setMpg(int percent) {
  int mpgoutput = map(percent, 0, 100, 255, 0);
...
void setTemp(int degrees) {
  ...
  // 50 - mid point
  // 25 - 
  // 10 - middle of red with light on
  // 5 - no better
  if (degrees < 50) { degrees = 50; }
  if (degrees > 120) { degrees = 120; }
  int tempoutput = map(degrees, 50, 120, 100, 10);
  analogWrite(tempPin, tempoutput);
}

void setRpm(int percent) {
  // 37  1000 - 0.037
  // 78  2000 - 0.039
  // 125 3000 - 0.0416
  // 176 4000 - 0.044
  ...
  // 273 6000 - 0.0455
  // 318 7000 - 0.0455
  int value = map(percent, 0, 100, 0, 318);
  setTach(value);
}

void setTach(int freq) {
  if (freq > 0) {
    rpmTone.play(freq);
  } else {
    rpmTone.stop();
...
void setup()
{
  ...
  // battery+oil (warning) lamps
  pinMode(10, OUTPUT);

  pinMode(fuelPin, OUTPUT);
  // pinMode(mpgPin, OUTPUT); // doesn't work
...
void CustomProtocolExample3_E36Speedo() {
  ...
  if (rpm > 90) {
    blinkWarningLeds = true;
    if (rpm > 95) {
      blinkTime = 100;
    } else {
      blinkTime = int((100 - rpm) * 40);
    }
  } else {
    blinkWarningLeds = false;
  }
  blinkLED();
  setRpm(rpm);
